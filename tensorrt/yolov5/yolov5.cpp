...
//    char *trtModelStream{ nullptr };
    ...
    cv::namedWindow("yolov5_result", CV_WINDOW_NORMAL);
    cv::moveWindow("yolov5_result", 730, 580);


}

void yolov5::Inference(cv::Mat video_frame) {
...
//    assert(outputIndex == 1);
    ...
    test_Inference(video_frame);


}

void yolov5::test_Inference(cv::Mat video_frame){
...
//    CHECK(cudaStreamCreate(&stream));
    ...
    if (!video_frame.empty()) {
        show_frame = video_frame.clone();
        fcount++;
//        std::cout<<"fcount: "<<fcount<<std::endl;
        for (int b = 0; b < fcount; b++) {
            cv::Mat pr_img = preprocess_img(video_frame); // letterbox BGR to RGB
            int i = 0;
        ...
        std::cout << "yolov5 inference frame time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                cv::Rect r = get_rect(show_frame, res[j].bbox);
                cv::rectangle(show_frame, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
                cv::putText(show_frame, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
            }

            cv::imshow("yolov5_result", show_frame);
//            cv::waitKey(10);
//            cv::imwrite("./result/_" + file_names[f - fcount + 1 + b], img);
        }
        fcount = 0;
    }
